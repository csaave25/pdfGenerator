...
export class PdfGenerateService {
  ...
  generateNewPage(espacio: number) {
    ...
    if ((this.totalY - this.usoPage) < espacio) {
      this.doc.addPage()
  ...
  generateFooter() {
    ...
    this.doc.text('1', this.midPointX, this.endY, { align: 'center' });
    this.doc.text("Néstor Ramírez Muena", 30, this.endY, { align: 'left' });
    this.doc.text("2023.12", 570, this.endY, { align: 'right' });
  }
  ...
  informacionTronadura(produccion: boolean, precorte: boolean, destape: boolean, bolones: boolean, desquinche: boolean, contorno: boolean,
    ...
    this.doc.text(volumen.toString(), calc + 20, alturaY + 75, { align: 'center' });
    this.doc.text("Factor de Carga General: " + factorCarga, calc * 2, alturaY + 75, { align: 'center' });
    // doc.text(form.factorCarga.value!?.toString(), calc * 2, 715, { align: 'center' });
  ...
  controlSector(radar: string, descripcionPunto: number, radarEste: number, radarNorte: number, radarCota: number, deplazamiento4horas: number, velocidadPromedio: number) {
    ...
    this.doc.setFontSize(8)
    this.doc.setFont("helvetica", "normal");
    this.doc.text("Figura 2: Mapa sintético de desplazamientos Vs. tiempo", this.midPointX, this.usoPage + 245, { align: 'center' });
    this.usoPage += 230
    ...
    this.doc.setFontSize(8)
    this.doc.setFont("helvetica", "normal");
    this.doc.text("Figura 3: Gráfico de desplazamientos Vs. tiempo", this.midPointX, this.usoPage + 255, { align: 'center' });
    this.usoPage += 230
  ...
  generarImagenes() {
    ...
    this.doc.text("Figura 4: Antes Panorámica ", 260 / 2 + this.left, this.usoPage + 200, { align: 'center' });

    //segunda img
    this.doc.addImage("assets/img1.png", 'PNG', this.right - 260, this.usoPage + 10, 260, 180, 'alias1', 'SLOW');
    this.doc.text("Figura 5: Después Panorámica", this.right - 260 / 2, this.usoPage + 200, { align: 'center' });
